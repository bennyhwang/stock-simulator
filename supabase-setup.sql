...
INSERT INTO stock_prices (symbol, name, base_price) VALUES
  ('0005.HK', '滙豐控股', 72.50), ('0700.HK', '騰訊控股', 380.00),
  ('9988.HK', '阿里巴巴', 95.20), ('0941.HK', '中國移動', 72.35),
  ('1299.HK', '友邦保險', 65.80), ('3690.HK', '美團', 185.50),
  ('1810.HK', '小米集團', 32.45), ('2388.HK', '中銀香港', 28.90),
  ('0001.HK', '長和', 52.60), ('0011.HK', '恒生銀行', 112.30),
  ('AAPL', 'Apple Inc.', 195.00), ('GOOGL', 'Alphabet Inc.', 175.50),
  ('MSFT', 'Microsoft Corp.', 425.00), ('TSLA', 'Tesla Inc.', 245.80),
  ('AMZN', 'Amazon.com', 185.30), ('NVDA', 'NVIDIA Corp.', 890.00)
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM(p.quantity * (
      SELECT ROUND(sp.base_price * (1.0 + (random() - 0.5) * 0.1), 2)
      FROM stock_prices sp WHERE sp.symbol = p.symbol
    )), 0) AS market_value,
    COALESCE(SUM(p.quantity * (
      SELECT ROUND(sp.base_price * (1.0 + (random() - 0.5) * 0.1), 2)
      FROM stock_prices sp WHERE sp.symbol = p.symbol
    ) - p.avg_cost), 0) AS total_pnl
  FROM traders t
